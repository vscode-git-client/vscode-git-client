...
### Fixed
- **Branch/tag commit-list open latency** — opening a branch or tag history no longer waits for a potentially slow branch refresh (including remote checks) before starting its `git log` query. The view uses cached refs for its initial render while the refresh continues in the background, matching Filter Graph's responsive loading path.
...
### Changed
- **`CommandController` refactored** — the 2424-line `register()` method was decomposed: 25 complex inline command callbacks were extracted into named private methods (`handleCherryPick`, `handleEditCommitMessage`, `handleOperationContinue`, `handleCompareWithRevision`, etc.), 21 closure-scoped type guards (`toBranchName`, `toCommitSha`, `toGraphCommitShas`, etc.) promoted to private methods, and 3 near-identical command groups consolidated (conflict resolution trio → `handleConflictResolve`, stash apply/pop → `handleStashApplyPop`, remote URL setter → `handleSetRemoteUrl`). The `register()` method shrank to ~826 lines (66% reduction) while preserving all 127 command IDs, legacy `intelliGit.*` aliases, and error-handling behaviour. See `docs/changelogs/command-controller-refactor.md` for the full phase-by-phase report.
- **Worktree creation flow** — Worktree creation now uses the branch/tag/revision picker for local branches, remote branches, tags, and typed revisions, then opens a folder picker for the destination path instead of asking users to type the path manually. Empty selected folders are used directly; non-empty selected folders are treated as parent folders and receive an auto-named child folder based on the current worktree folder and selected ref, with a clear error if that child path already exists.
