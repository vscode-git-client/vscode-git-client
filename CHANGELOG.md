...
- **Filter Graph logic fixes** — branch-filtered results no longer get incorrectly narrowed again by decorated refs in the webview, stale scroll-pagination responses no longer append into newer filter sessions, empty states remain visible during pagination, and multi-selected commits now keep supported copy actions enabled in the shared context menu.
