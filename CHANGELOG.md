...
- **Commit Details range selected-change actions** — merged/range diffs opened from Compare Branches or Filter Graph now expose the same multi-selected change actions as normal commit diffs: `Open Diffs`, `Revert Selected Changes`, `Cherry-pick Selected Changes`, and `Create Patch from Selected Changes`.
...
- **Commit Details working-tree compare context menu** — file rows shown after comparing a folder with a revision now expose `Open Diffs`, `Revert Selected Changes`, and `Create Patch from Selected Changes`, with multi-selection support.
