...
export class TextCompareSession implements vscode.Disposable {
  ...
  private async open(left: TextSource, right: TextSource): Promise<void> {
    ...
    try {
      ...
      diffTab = findDiffTab(this.leftUri, this.rightUri);
  ...
  private async disposeIfHidden(diffTab?: vscode.Tab): Promise<void> {
    ...
    const diffStillOpen = diffTab ? tabExists(diffTab) : false;
    const leftStandalone = this.leftUri ? isUriVisibleStandalone(this.leftUri) : false;
    const rightStandalone = this.rightUri ? isUriVisibleStandalone(this.rightUri) : false;

    if (!diffStillOpen || leftStandalone || rightStandalone) {
...
function tabExists(tab: vscode.Tab): boolean {
  return vscode.window.tabGroups.all.some((group) => group.tabs.includes(tab));
}

function isUriVisibleStandalone(uri: vscode.Uri): boolean {
