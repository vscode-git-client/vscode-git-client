      ...
      getRemoteSourceActions: () => ([
        {
          ...
          icon: 'tools',
          run(branch: string): void {
            void vscode.commands.executeCommand(CommandId.BranchActionHub, branch);
          }
        }
      ])
    });
...
function attachOperationStatusBarActions(
  ...
  continueItem.command = CommandId.OperationContinue;
  ...
  abortItem.command = CommandId.OperationAbort;
  ...
  skipItem.command = CommandId.OperationSkip;
